Create a single horizontal sprite strip for the Codex app digital pet `danling` in the state `running-left`.
...
DanLing state meaning: this row also supports `normal` directional movement. Keep it steady and confident, matching the rightward row if mirrored or generated.
