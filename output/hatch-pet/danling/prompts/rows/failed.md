Create a single horizontal sprite strip for the Codex app digital pet `danling` in the state `failed`.
...
DanLing state meaning: this row represents `failed`, when metrics contain NaN or inf. Show a compact failed cultivation-state reaction: slumped posture, dim expression, maybe tiny attached hard-edged stars or smoke touching the head or robe, but no detached symbols.
