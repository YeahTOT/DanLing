Create a single horizontal sprite strip for the Codex app digital pet `danling` in the state `idle`.
...
DanLing state meaning: this row represents `idle`, when there is currently no loss and no score. Keep 丹灵 calm, neutral, and quietly waiting for the first metric snapshot.
