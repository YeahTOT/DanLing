Create a single horizontal sprite strip for the Codex app digital pet `danling` in the state `waving`.
...
DanLing state meaning: this row represents `happy`, when the recent loss-window slope is decreasing. Show a small pleased sleeve/hand wave and brighter face, still subtle and sprite-like; no wave marks, sparkles, text, or floating effects.
