Create a single horizontal sprite strip for the Codex app digital pet `danling` in the state `running`.
...
DanLing state meaning: this row represents `anxious`, when score has not improved for no_improve_patience snapshots. Show busy non-locomotion processing with tense focus, small fidgeting hands, and a worried face; do not show literal running or directional travel.
