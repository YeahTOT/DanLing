Create a single horizontal sprite strip for the Codex app digital pet `danling` in the state `waiting`.
...
DanLing state meaning: this row represents `sleeping`, when the latest log update is older than stale_seconds. Show a sleepy stale-log state with drooping eyelids and a gentle drowsy sway; no floating Z letters, no text, no bubbles.
