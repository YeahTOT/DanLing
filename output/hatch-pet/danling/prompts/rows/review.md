Create a single horizontal sprite strip for the Codex app digital pet `danling` in the state `review`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: a compact chibi pixel-art xianxia cultivator inspired by Han Li, long black hair tied high with a white ribbon, calm grey eyes, white and pale-grey hanfu robe with dark teal trim and belt, small jade-blue pendant, tiny hands, sturdy readable silhouette for the DanLing project; the attached conversation reference image defines the desired look.
...
DanLing state meaning: this row represents `sick`, when current loss is more than 2x the previous valid loss. Show careful worried inspection: slightly hunched, concerned eyes, one hand near the robe or pendant as if checking unstable qi, but no UI, no papers, no symbols.
